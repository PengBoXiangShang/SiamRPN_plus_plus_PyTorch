...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(
            block, planes[0], layers[0], groups=groups, norm_layer=norm_layer, padding=1, dilation_ratio=1)
        self.layer2 = self._make_layer(
            block, planes[1], layers[1], stride=2, groups=groups, norm_layer=norm_layer, padding=1, dilation_ratio=1)
        # TODO
        self.extra_1x1_conv3 = nn.Conv2d(
            512, 256, kernel_size=1, stride=1, padding=0, bias=False)
        self.layer3 = self._make_layer(
            block, planes[2], layers[2], stride=1, groups=groups, norm_layer=norm_layer, padding=2, dilation_ratio=2)
        # TODO
    ...
    def _make_layer(self, block, planes, blocks, stride=1, groups=1, norm_layer=None, padding=1, dilation_ratio=1):
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        downsample = None
